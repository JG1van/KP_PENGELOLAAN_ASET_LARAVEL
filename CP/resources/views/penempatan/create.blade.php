                    ...
                    <div class="scroll-box" style="max-height: 400px; overflow-y: auto;">
                        @foreach ($lokasi as $lok)
                            <table class="table table-bordered table-hover align-middle text-center lokasi-tabel d-none"
                                ...
                                <tbody id="kiri-tbody">
                                    @forelse ($aset->filter(fn($a) => $a->penempatanTerakhir
                        && $a->penempatanTerakhir->Id_Lokasi == $lok->Id_Lokasi
                        && $a->penempatanTerakhir->Id_Lokasi != 0) as $a)
                    ...
                    <div class="scroll-box" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            <tbody id="kanan-tbody">
                                @foreach ($aset->filter(fn($a) => !$a->penempatanTerakhir || $a->penempatanTerakhir->Id_Lokasi == 0)->sortBy('Id_Aset') as $a)
                                    <tr id="row-{{ $a->Id_Aset }}" data-id="{{ $a->Id_Aset }}">
                                        <td>{{ $a->Id_Aset }}</td>
                                        <td>{{ $a->Nama_Aset }}</td>
                                        <td>
                                        ...
                                        <input type="hidden" name="penempatan[{{ $a->Id_Aset }}][Id_Aset]"
                                            value="{{ $a->Id_Aset }}">
                                        <input type="hidden" name="penempatan[{{ $a->Id_Aset }}][Id_Lokasi]"
                                            value="1" class="input-lokasi-default">
                                    </tr>
                                @endforeach
                            </tbody>
...
@section('js')
    <script>
        ...
        @endforeach`;
            ...
            <input type="hidden" name="penempatan[${asetId}][Id_Aset]" value="${asetId}">
            <input type="hidden" name="penempatan[${asetId}][Id_Lokasi]" value="1" class="input-lokasi-default">
        `;
